            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('reservations.update', $reservation->id) }}">
                        @csrf
                        @method('PUT') <!-- Especificamos que el método es PUT para la actualización -->

                        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                            <!-- Fecha de Reserva -->
                            <div>
                                <x-label for="reservation_date" :value="__('Fecha de Reserva')" />
                                <x-input id="reservation_date" class="block mt-1 w-full" type="date" name="reservation_date" :value="old('reservation_date', $reservation->reservation_date->toDateString())" required />
                            ...
                            <div>
                                ...
                                <select id="time_slot_id" name="time_slot_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                                    ...
                                    @foreach($timeSlots as $timeSlot)
                                        <option value="{{ $timeSlot->id }}" {{ old('time_slot_id', $reservation->time_slot_id) == $timeSlot->id ? 'selected' : '' }}>
...
<div>
    ...
    <select id="zone_id" name="zone_id" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
        ...
        @foreach($zones as $zone)
            <option value="{{ $zone->id }}" {{ old('zone_id', $reservation->zone_id) == $zone->id ? 'selected' : '' }}>
                {{ $zone->name }}
            </option>
        @endforeach
    </select>
    ...
    @enderror
                            ...
                            <div>
                                ...
                                <x-input id="guest_count" class="block mt-1 w-full" type="number" name="guest_count" :value="old('guest_count', $reservation->guest_count)" min="1" required />
                                @error('guest_count')
                                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea id="special_requests" name="special_requests" class="block mt-1 w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">{{ old('special_requests', $reservation->special_requests) }}</textarea>
